    ...
    public partial class ProductService : IProductService
    {
        ...
        public virtual IList<Product> GetAllProductsDisplayedOnHomePage()
        {
            var query = from p in _productRepository.Table
                        orderby p.CreatedOnUtc descending, p.Id
                        where p.Published &&
                        !p.Deleted &&
                        p.ShowOnHomePage
                        select p;
        ...
        /// <summary>
        /// Get products by customer email address
        /// </summary>
        /// <param name="custEmailAddress">Customer Email Address</param>
        /// <returns>Product</returns>
        public virtual Product GetProductsByCustomerEmailAddress(string emailAddress)
        {
            if (emailAddress == null || emailAddress.Length == 0)
                return new Product();

            var query = from p in _productRepository.Table
                        where p.ShortDescription.Contains(emailAddress) && !p.Deleted
                        select p;
            var product = query.ToList().SingleOrDefault();
