...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual ActionResult Register(RegisterModel model, string returnUrl, bool captchaValid, FormCollection form)
        {
            ...
            //Changes related to vendor registartaion start
            if (model.Category != null && model.Category != "1")
            {
                //Add a new vendor for the registered user and set vendor id in the customer table
                Nop.Core.Domain.Vendors.Vendor vendor = new Nop.Core.Domain.Vendors.Vendor();
                vendor.Name = model.FirstName + model.LastName + "_Vendor";
                vendor.Email = model.Email;
                vendor.Description = model.FirstName + model.LastName + "_Description";
                vendor.PictureId = 0;
                vendor.Deleted = false;
                vendor.Active = true;
                vendor.DisplayOrder = 1;
                vendor.PageSize = 6;
                vendor.AllowCustomersToSelectPageSize = true;
                vendor.PageSizeOptions = "6, 3, 9, 18";

                //Insert Vendor
                _vendorService.InsertVendor(vendor);

                //Get the latest inserted vendor id
                int vendorID = _vendorService.GetAllVendors().OrderByDescending(x => x.Id).FirstOrDefault().Id;
                if (vendorID != 0)
                {
                    customer.VendorId = vendorID;
                }

                //Add newly added BMY Teacher/Studio as a product under Yoga Expert Catergory
                Nop.Core.Domain.Catalog.Product product = new Nop.Core.Domain.Catalog.Product();
                product.ProductTypeId = 5;
                product.ParentGroupedProductId = 0;
                product.VisibleIndividually = true;
                product.Name = model.FirstName + " " + model.LastName;
                product.VendorId = vendorID;
                
                string registrationType = model.Category == "2" ? "Yoga Trainer" : model.Category == "3" ? "Yoga Institute" : "Yoga Studio";

                product.ShortDescription = registrationType + " Mobile: " + model.Phone + " E-mail: " + model.Email;
                product.FullDescription = @"<p class=""card-title""><span id=""ContentPlaceHolder1_category"">"+ registrationType +"</span></p>" +
                                           @"<p class=""card-title""><span id=""ContentPlaceHolder1_mobile"">Mobile: " + model.Phone + "</span></p>" +
                                           @"<p class=""card-title""><span id=""ContentPlaceHolder1_email"">E-mail: " + model.Email + "</span></p>" +
                                           @"<p class=""card-title""><span id=""ContentPlaceHolder1_address"">Address: " + model.City + ", " + model.StreetAddress + ", " + model.ZipPostalCode + "</span></p>";
                ...
                product.Sku = model.FirstName + " " + model.LastName;
                ...
                product.MaximumCustomerEnteredPrice = 10000;
                ...
                //categories
                SaveCategoryMappings(product);
            }
          
            //Changes related to vendor registartaion end
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    //Changes under registartion on 08/01/2019 start
                    if (model.Category != null)
                        _genericAttributeService.SaveAttribute(customer, SystemCustomerAttributeNames.Category, model.Category);
                    //Changes under registartion on 08/01/2019 end
        ...
        [NonAction]
        protected virtual void SaveCategoryMappings(Product product)
        {
        ...
        public virtual ActionResult UploadAvatar(CustomerAvatarModel model, HttpPostedFileBase uploadedFile)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    //Code changes to add/update avator image in the product as well in case of yoga trainer/studio/teacher START
                    var product = _productService.GetProductsByCustomerEmailAddress(customer.Email);
                    if(product != null)
                    {
                      var productPicture = _productService.GetProductPicturesByProductId(product.Id).SingleOrDefault();
                       if(productPicture != null)
                       {
                           productPicture.PictureId = customerAvatar.Id;
                           _productService.UpdateProductPicture(productPicture);
                       }
                       else
                       {
                           ProductPicture newProductPicture = new ProductPicture();
                           newProductPicture.ProductId = product.Id;
                           newProductPicture.PictureId = customerAvatar.Id;
                           newProductPicture.DisplayOrder = 0;
                           _productService.InsertProductPicture(newProductPicture);
                       }
                    }
                    //Code changes to add/update avator image in the product as well in case of yoga trainer/studio/teacher END
