...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        protected virtual void PrepareCategoryMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            ...
            //Vendor will have access to all product categories except Yoga Teachers & Experts and Yoga Products
            if ( _workContext.CurrentCustomer.VendorId != 0)
            {
                foreach (var c in allCategories.Where(x => x.Value != "15" && x.Value != "18")) //Where Category Yoga Products = 15,Yoga Teachers & Experts = 18
                {
        ...
        public virtual ActionResult List()
        {
            ...
            if (_workContext.CurrentCustomer.VendorId != 0)
            {
                foreach (var c in categories.Where(x => x.Value != "15" && x.Value != "18")) //Where Category Yoga Products = 15,Yoga Teachers & Experts = 18
                {
                    model.AvailableCategories.Add(c);
                }
            }
